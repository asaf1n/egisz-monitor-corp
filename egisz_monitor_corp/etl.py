...
Архитектура `run_sync` (сначала выгрузка из FB, затем парсинг/UPSERT):
  1. `_export_egisz_licenses_full` — полная выгрузка EGISZ_LICENSES + JOIN JPERSONS из Firebird; запись сырого снимка в
     `stg_egisz_licenses_import`, merge в `dim_clinics` и отбор строк с JID в PostgreSQL; при dry_run — тот же отбор в Python.
  2. `_count_exchangelog_total` — заглушка для прогресса UI (без COUNT в Firebird).
  3. `_export_egisz_messages_by_egmid` — постраничная выгрузка EGISZ_MESSAGES только по EGMID выше курсора; в PG сразу обновляется
     `etl_state.source_max_egmid` (пик выгрузки для UI); `last_egmid` — только после полного успешного sync.
  4. `_process_exchangelog_pages` — для каждой страницы: выборка EXCHANGELOG из FB по LOGID, затем склейка
     по MSGID с выгруженными сообщениями, парсинг, UPSERT; один снимок прогресса на страницу после выборки из FB.
...
def _fetch_egisz_licenses_raw_from_firebird(
    ...
    log("Firebird: выгрузка EGISZ_LICENSES (JOIN JPERSONS), полный снимок…")
    rows = _etl_fb_fetch(cfg, enrichment_egisz_licenses_sql())
    log(f"EGISZ_LICENSES: прочитано {len(rows)} строк из Firebird.")
    return rows
