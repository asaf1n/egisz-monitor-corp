...
- **Сервисы:**
  ...
  - **Jobs:** `egisz-reports-schema-init` применяет **`sql/001_schema.sql`**, **`sql/002_etl_state.sql`**, **`sql/005_healthcheck.sql`** (ConfigMap `egisz-reports-schema` создаётся из `start.ps1`). Отдельно: Airflow metadata init и др. в `k8s/postgres/`.
