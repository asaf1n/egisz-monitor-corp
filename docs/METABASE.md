...
**Доступ с ПК (порт 3000):** в `k8s/metabase.yaml` объявлен Service **`metabase`** (`type: LoadBalancer`), на Docker Desktop он обычно публикует **[http://127.0.0.1:3000/](http://127.0.0.1:3000/)** — без `kubectl port-forward`. Если `LoadBalancer` в Pending (например kind) — `kubectl -n egisz-monitor port-forward svc/metabase 3000:3000` или `.\start.ps1 -Action web`. В Deployment **нет `hostPort`** (иначе на однонодовом кластере новый под Metabase часто зависает в **Pending** из‑за занятого порта 3000 на ноде). После `.\start.ps1 -Action apply` по умолчанию выполняется **`rollout restart` Metabase** — холодный JVM, миграции app DB и readiness; это **2–6+ минут** нормально. Только правки Config UI: **`.\start.ps1 -Action apply -SkipMetabaseRolloutRestart`**.
