...
4. **Устаревший кэш на ноде:** в `k8s/metabase.yaml` задан версионируемый тег **`egisz-monitor-metabase:k8s-v10`** (не `latest`): при смене JSON/скриптов **увеличьте** тег в манифесте и в `start.ps1` (`docker tag … :k8s-v…`), затем `kubectl apply -f k8s/metabase.yaml` и **`rollout restart`** — иначе Docker Desktop может оставить старый digest. Дополнительно `.\start.ps1 -Action build` создаёт `:local` для `provision-local.ps1`. Если «no such file» к verify: `build` + `apply`, либо `.\metabase\force-k8s-mb-image.ps1`.
