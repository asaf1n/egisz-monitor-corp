...
4. **Устаревший кэш на ноде:** в `k8s/metabase.yaml` — **`egisz-corp-metabase:local`**, `IfNotPresent` (не `latest`: на ноде тогда оставался чужой digest, без `/app/verify-corp-stack.sh`). После `.\start.ps1 -Action build` (создаётся `:local`) `apply`/`deploy` делают **`rollout restart`** deployment/metabase на Docker Desktop — новый pod берёт **актуальный** digest по тегу `local`. Если снова «no such file» к verify: `build` + `apply`, либо `.\metabase\force-k8s-mb-image.ps1`.
