...
| Каждый **apply** долго | `apply` всегда перезапускает Metabase и conf-ui. Только правки Flask / синка без полного `apply`: **`.\start.ps1 -Action restart-web`**. После смены JSON дашбордов: **`.\start.ps1 -Action restart-metabase`** (пересборка образа + `kind load` + rollout). |
...
3. **Metabase** смотрит на ту же БД `egisz_reports` и только **читает** витрину. После смены JSON-дашбордов или `Dockerfile` Metabase: **`docker build -f metabase/Dockerfile`**, бамп тега **`egisz-monitor-metabase:k8s-v16`** (см. `k8s/metabase.yaml`), **`.\start.ps1 -Action restart-metabase`** или полный **`apply`** (подхватит образ; если в БД metabase уже есть 11 EGISZ-дашбордов с именами «01 …» — провижининг **пропускается**, dashboard ID не меняются). Если нужно принудительно пересобрать дашборды: **`deploy`** / **`reset-deploy`** (DROP/CREATE БД `metabase`) или временно **`METABASE_FORCE_PROVISION=true`** в `k8s/metabase.yaml`. Локальный kind: **`restart-metabase`** / **`deploy`** выполняют `kind load` внутри `start.ps1`. При **только** обновлении **витрины** без смены образа Metabase достаточно повторного **apply-schema** и/или ETL; пересобирать образ Metabase **не** обязательно.

**Разделители в «Сводка ошибок»:** между элементами массива `errors_json` у **одного документа колбэка** (один `relates_to_id` во витрине) — **·** (средняя точка); внутри многочастевого Schematron в одном `message` — **—** (короткое тире в SQL). Понятные бизнес-сообщения ГИП (`не соответствует данным ГИП` и т.д.) **не** перезаписываются.

## Пример вопроса (SQL)

Агрегаты по витрине колбэков обычно считают **уникальные документы**, а не строки журнала: `COUNT(DISTINCT "Связанное сообщение")` (= `relates_to_id`).

```sql
SELECT "Статус", COUNT(DISTINCT "Связанное сообщение")::bigint AS "Документов"
...
4. **Устаревший кэш на ноде:** в `k8s/metabase.yaml` задан версионируемый тег **`egisz-monitor-metabase:k8s-v16`** (не `latest`): при смене JSON/скриптов **увеличьте** тег в манифесте и в `start.ps1` (`docker tag … :k8s-v…`), затем `kubectl apply -f k8s/metabase.yaml` и **`rollout restart`** — иначе Docker Desktop может оставить старый digest. Тег **`egisz-monitor-metabase:local`** создаётся при **`deploy`/`reset-deploy`/`restart-metabase`** и в **`metabase/provision-local.ps1`** для локального провижининга. Если «no such file» к verify: **`restart-metabase`** + **`apply`**, либо `.\metabase\force-k8s-mb-image.ps1`.
