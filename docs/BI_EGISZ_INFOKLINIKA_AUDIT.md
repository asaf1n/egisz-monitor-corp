...
| **Отказы контента РЭМД** | Ответ шлюза со статусом ошибки и массивом сообщений проверки. | `errors_json`, функции `egisz_error_interpretation_*` — дашборды **04**, **01**, **05**. |
...
| Объект | Назначение |
|--------|------------|
| `v_egisz_transactions_enriched_ui` | Основная витрина с русскими колонками; **«Интерпретация ошибок»** (и устаревшая «Сводка ошибок»), **«Ошибки JSON»**. |
...
| `egisz_error_interpretation_item`, `egisz_error_interpretation_row`, `egisz_error_interpretation_type` | Интерпретация сообщений РЭМД по отказам без искажения сырого ответа (`errors_json`); нормализация переменных для группировок — `*_type`. |
...
| :--- | :--- | :--- |
| Данные и витрина | Высокая | Расширять `egisz_error_interpretation_*` под новые типы отказов РЭМД (официальный источник кодов — НСИ 1.2.643.5.1.13.13.99.2.305). |
