...
| `etl.py` | **`run_sync`**: справочники первыми; **чередование** пакетов **EXCHANGELOG** (`LOGID > last_log_id`, без JOIN к FB `EGISZ_MESSAGES`) и страниц снимка **EGISZ_MESSAGES** в **`stg_egisz_messages_journal`** (keyset **`EGMID > after_egmid`**); при **`sync_window_days` > 0** — **единая логика окна**: предикаты **`LOGDATE`** / **`CREATEDATE`** в Firebird и **сброс `last_log_id` и `last_egmid` в 0 в начале каждого run** (перечитывание обеих таблиц внутри окна с минимального keyset, догрузка пропусков); при **0** — инкремент **без дат** по сохранённым курсорам; при **< 0** — полный пересъём без дат: **сброс `last_log_id` и `last_egmid` в 0** (staging снимка **не** TRUNCATE, UPSERT по **`MSGID`**); при сохранении YAML в Config UI с **`sync_window_days` < 0** — тот же **сброс курсоров** в **`etl_state`** (если PostgreSQL доступен); перед разбором пакета журнала — **догрузка** **`EGISZ_MESSAGES` по `MSGID`**, отсутствующих в staging; сопоставление журнала с сообщениями в PG по **`MSGID`**; UPSERT фактов; **исходящие** — полная перезапись **`stg_egisz_outbound_documents`** в том же окне **`CREATEDATE`**; прогресс UI: **`documents_*`**, **`outbound_*_docs`**; без COUNT журнала в Firebird; `pg_try_advisory_lock` |
| `parser.py` | **`EgiszMonitorParser`**: разбор MSGTEXT (SOAP/XML), `relates_to_id`, `status`, `errors_json`, `localUid`, `emdr_id`, `resolve_clinic`; без `relatesToMessage` факт не строится. **Шумоподавление:** SOAP/XML фрагменты без признаков callback РЭМД (нет `registerDocumentResult`/callback-namespace) не пишутся в `stg_parse_errors`; `MISSING_RELATES_TO` фиксируется только для callback‑похожих фрагментов. Без **localUid**/DOCUMENTID и без **emdrId** при наличии `relatesToMessage` — `stg_parse_errors` (`MISSING_DOCUMENT_IDENTIFIERS`), факт не создаётся |
...
3. **Дашборды** — править JSON в `metabase_dashboards/` (включая блок healthcheck в `02_service.json`), бамп тега Metabase в `start.ps1` + `k8s/metabase.yaml`, пересборка/выкат — см. `README.md`.
4. **Healthcheck** — `sql/005_healthcheck.sql`, `pg_warehouse.fetch_healthcheck_snapshot`, эндпоинт `/api/healthcheck`, JS-блок в `config_app.py`, карточки healthcheck в `02_service.json`. Пороги — в CTE `params` и комментариях `sql/005_healthcheck.sql`.
...
Тексты в **`README.md`**, **`AGENTS.md`**, **`.cursorrules`** читают инженеры вне контекста чата. Соблюдай:
