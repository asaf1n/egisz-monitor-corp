...
| **07 Глубокий анализ ошибок** | **Классификация инцидентов.** Группировка текстов ошибок из JSON-ответов РЭМД. Позволяет выявить системные проблемы в заполнении данных для исправления в МИС. | `SELECT error_message, COUNT(*) FROM public.v_parse_errors_deep GROUP BY 1 ORDER BY 2 DESC` |
| **08 Агрегация ожидающих** | **Мониторинг задержек.** Распределение документов в статусе «ожидание» по клиникам. Позволяет приоритизировать работу по восстановлению синхронизации в проблемных точках. | `SELECT "Наименование клиники", COUNT(*) FROM public.v_rpt_pending_agg GROUP BY 1` |
...
| **10 Топ ошибок** | **Аналитика проблем.** Рейтинг наиболее частотных ошибок парсинга и валидации для формирования бэклога задач на доработку МИС или ETL-сервиса. | `SELECT LEFT(error_msg, 100), COUNT(*) FROM public.stg_parse_errors GROUP BY 1 ORDER BY 2 DESC` |
