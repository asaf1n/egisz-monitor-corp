...
**Порядок шага синхронизации:** (1) из Firebird одним проходом подгружаются справочники **`EGISZ_LICENSES`** и **`JPERSONS`** для обогащения и маппинга в памяти процесса; (2) при необходимости выполняется **COUNT** строк журнала после курсора (для прогресса в UI); (3) **`EXCHANGELOG`** выбирается **постранично** (`batch_size`, обёртка `SELECT FIRST …` с сортировкой по `LOGID` в `sql_util.paginated_exchangelog_sql`); (4) для каждой строки парсер разбирает **`MSGTEXT`** / **`LOGTEXT`**, ошибки парсинга пишутся в staging в PG; (5) факты и измерения **UPSERT** в витрину, после обработки пакета курсор **`last_log_id`** обновляется до максимального **`LOGID`** на странице; (6) отдельным запросом к Firebird обновляется staging **исходящих документов** (`stg_egisz_outbound_documents` / отчёт «без ответа»).
