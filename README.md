...
Центральная процедура — **`run_sync`** в модуле `egisz_monitor_corp.etl`: перенос данных **Firebird → PostgreSQL** без отдельного CDC-сервиса, только **SELECT** через **firebird-driver**.

**Инкремент и курсор:** основной ориентир — поле **`LOGID`** в `EXCHANGELOG` (не `MODIFYDATE`). В PostgreSQL в таблице **`etl_state`** хранится **`last_log_id`** для имени пайплайна из конфигурации (по умолчанию `firebird_exchangelog`). При **`full_scan: true`** курсор сбрасывается и выполняется полный проход в рамках настроенного окна. В обычном режиме читаются строки с **`LOGID` больше сохранённого курсора**.
...
* **Курсор (watermark) по LOGID:** это не «offset страницы», а последний обработанный **`LOGID`**: значение читается из таблицы **`etl_state`** в PostgreSQL и после каждого пакета сдвигается вперёд (до максимума **`LOGID`** на странице).
...
* **Объекты кэширования:** При старте задачи синхронизации из Firebird выбираются строки **`EGISZ_LICENSES`** с непустым **`JID`** и строки **`JPERSONS`** с непустым **`JID`** (см. `enrichment_*_sql` в `sql_util.py`), а не обязательно целые таблицы без фильтра.
...
* Асинхронная связь ответа с исходным сообщением задаётся тегом **`<relatesToMessage>`** в XML из **`EXCHANGELOG.MSGTEXT`** → поле витрины **`relates_to_id`** (без него факт не пишется, ошибка может уйти в **`stg_parse_errors`**).
...
1. **По URL / REPLYTO:** токен **`gost-<jid>.infoclinica.lan`** в **`EXCHANGELOG.LOGTEXT`**, при отсутствии — в **`EGISZ_MESSAGES.REPLYTO`** (регулярное выражение в парсере, схема `http://` не обязательна).
...
4. **Наименование и реквизиты** из **`JPERSONS`** подставляются в измерения **после** того, как **`JID`** уже известен (это не отдельный способ вычислить `JID`).
...
| **03 Ошибки и разбор** | **Технический аудит.** Реестр **`stg_parse_errors`** (нет `relatesToMessage`, битый XML и т.п.); отказы РЭМД по документам смотрите во витрине транзакций (`errors_json` / соответствующие отчёты). | `SELECT id, error_code, LEFT(message, 200), created_at FROM public.stg_parse_errors ORDER BY id DESC LIMIT 100` |
...
| **Config UI** | `conf-ui:8080` | Веб-интерфейс конфигурации (Flask); синхронизация — тот же **`run_sync`**, что и у CLI **`egisz-corp sync`**, через API UI (не отдельный под с CLI). |
