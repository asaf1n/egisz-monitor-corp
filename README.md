...
* **Apache Airflow**: Оркестрация ETL-процессов (DAG `egisz_corp_etl_dag`).
* **Pydantic v2**: Валидация конфигураций и данных.
* **SQLAlchemy / Psycopg2**: Взаимодействие с PostgreSQL.
* **fdb**: Библиотека для работы с СУБД Firebird.

**Хранение данных:**
* **Firebird**: Источник сырых данных (транспортные логи МИС).
* **PostgreSQL 15/16**: Аналитическое хранилище (DWH) и хранение состояний ETL.

**Визуализация и мониторинг:**
* **Metabase**: BI-платформа для оперативной и управленческой отчетности (9 преднастроенных дашбордов).

**Инфраструктура и UI:**
* **FastAPI / Uvicorn**: Сервис для управления конфигурациями (Config UI).
* **Docker / Docker Compose**: Контейнеризация всех компонентов стека.
* **Kubernetes (k8s)**: Манифесты для развертывания в корпоративном кластере (Namespace `egisz-corp`).
