...
WITH facts AS (
    SELECT
        f.jid,
        COUNT(*) FILTER (WHERE f.processed_at > NOW() - INTERVAL '24 hours') AS facts_24h,
        COUNT(*) FILTER (WHERE f.status = 'error'   AND f.processed_at > NOW() - INTERVAL '24 hours') AS errors_24h,
        COUNT(*) FILTER (WHERE f.status = 'unknown' AND f.processed_at > NOW() - INTERVAL '24 hours') AS unknown_24h,
        COUNT(*) FILTER (WHERE f.status = 'success' AND f.processed_at > NOW() - INTERVAL '24 hours') AS success_24h,
...
queue AS (
    SELECT jid, COUNT(*)::bigint AS pending_now
...
COMMENT ON VIEW v_health_by_clinic IS 'Healthcheck по клиникам: объём за 24ч, error/unknown rate, текущая очередь без ответа, цветовой уровень health_level (green/yellow/red).';
...
agg AS (
    SELECT
        (SELECT COUNT(*) FROM fact_egisz_transactions WHERE processed_at > NOW() - INTERVAL '24 hours') AS facts_24h,
        (SELECT COUNT(*) FROM fact_egisz_transactions WHERE processed_at > NOW() - INTERVAL '24 hours' AND status = 'error') AS errors_24h,
        (SELECT COUNT(*) FROM fact_egisz_transactions WHERE processed_at > NOW() - INTERVAL '24 hours' AND status = 'unknown') AS unknown_24h,
        (SELECT COUNT(*) FROM stg_parse_errors WHERE created_at > NOW() - INTERVAL '1 hour') AS parse_errors_1h,
        (SELECT COUNT(*) FROM v_rpt_documents_no_response r WHERE r.sent_at < NOW() - INTERVAL '24 hours') AS queue_older_24h,
...
SELECT
    ...
    'rows/hour',
    NULL,
    'Если > порога: SELECT * FROM stg_parse_errors ORDER BY id DESC LIMIT 50.'
FROM agg a CROSS JOIN params p
UNION ALL
SELECT
    'queue_red_24h',
...
SELECT
    ...
    (SELECT COUNT(*)::bigint FROM v_rpt_documents_no_response) AS pending_total,
    (SELECT COUNT(*)::bigint FROM v_rpt_documents_no_response WHERE sent_at >= NOW() - INTERVAL '1 hour') AS pending_1h,
    (SELECT COUNT(*)::bigint FROM v_rpt_documents_no_response WHERE sent_at >= NOW() - INTERVAL '24 hours' AND sent_at < NOW() - INTERVAL '1 hour') AS pending_1_24h,
    (SELECT COUNT(*)::bigint FROM v_rpt_documents_no_response WHERE sent_at < NOW() - INTERVAL '24 hours') AS pending_older_24h,
    (SELECT MAX(updated_at) FROM etl_state) AS etl_last_update,
...
SELECT
    ...
    facts_24h                       AS "Транзакций за 24ч",
    ...
    pending_now                     AS "В очереди сейчас",
...
INSERT INTO dim_column_display_labels (source_object, source_column, display_label_ru) VALUES
    ...
    ('v_health_by_clinic', 'facts_24h', 'Транзакций за 24ч'),
