...
COMMENT ON COLUMN etl_state.last_egmid IS 'Ватермарк EGMID после полного успешного sync (журнал + outbound); при сбое не двигается, чтобы повторить выгрузку сообщений для MSGID';
...
COMMENT ON COLUMN etl_state.source_max_egmid IS 'Пик EGMID после выгрузки EGISZ_MESSAGES из Firebird (обновляется до парсинга журнала; в UI max с last_egmid)';
...
COMMENT ON COLUMN etl_state.source_peaks_updated_at IS 'Когда записали source_max_* из Firebird';
